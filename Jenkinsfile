  ...
  stages {
    ...
    stage ('Deploy DVNA to Production') {
      steps {
        ...
        sh 'ssh -o StrictHostKeyChecking=no prod-vm@192.168.1.39 "docker run -d --name dvna-app --env-file ~/vars.env --link dvna-mysql:mysql-db -p 9090:9090 appsecco/dvna"'
      }
    }
